...
class NetworkExecutor {
    ...
    func performOperation<Output: Codable>(
        ...
        guard let endpoint = NetworkExecutor.endpoint, !endpoint.isEmpty else {
            completion(
                ResultModel.error(
                    ErrorModel(message: "Please provide a valid endpoint", errorType: "NETWORK")))
            return
        }
        ...
        guard let apiKey = NetworkExecutor.apiKey, !apiKey.isEmpty else {
            completion(
                ResultModel.error(
                    ErrorModel(message: "Please provide a valid apiKey", errorType: "NETWORK")))
            return
        }
        ...
        do {
            ...
            let task = URLSession.shared.dataTask(with: request) { data, _, error in
                if let error = error {
                    completion(
                        ResultModel.error(
                            ErrorModel(message: error.localizedDescription, errorType: "UN_KNOWN")))
                    return
                }

                guard let data = data else {
                    completion(
                        ResultModel.error(ErrorModel(message: "No data received", errorType: "NETWORK")))
                    return
                }
                ...
                do {
                    ...
                    if let errors = responseModel.errors {
                        completion(
                            ResultModel.error(
                                ErrorModel(
                                    message: errors.first?.message, errorType: errors.first?.extensions.classification
                                )))
                        ...
                        completion(
                            ResultModel.error(
                                ErrorModel(
                                    message: "There’s an issue. Please try again later", errorType: "NETWORK")))
                    }
                } catch {
                    completion(
                        ResultModel.error(
            ...
            completion(
                ResultModel.error(
                    ErrorModel(message: error.localizedDescription, errorType: "REQUEST_CREATION")))
        }
    }
}
